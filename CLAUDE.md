...
- `chatadmin_content_backends` — register `ContentBackend` implementations (`includes/class-content-backends.php`). The default `WPContentBackend` handles `wp_post` / `wp_page_slug` / `wp_post_meta` / `wp_term`. `ContentRouter` dispatches each tool call to whichever backend claims the target `kind`. Custom backends (adding their own kinds, optionally implementing `search()` for `find_text`) live in separate site plugins.
