...
- License storage reuses the `chatadmin_settings` option + the `CHATADMIN_{PROVIDER}_API_KEY` constant-precedence pattern (`CHATADMIN_CLOUD_LICENSE`).
- A `chatadmin_cloud_http_response` test seam + `tests/MockCloud.php` mirror the `chatadmin_anthropic_http_response` pattern, so Cloud is deterministic and free in tests.
...
- One **Product** "ChatAdmin Cloud" + one **recurring Price** €12/mo. Consider a `trial_period_days` (open question). Stripe account is live ("Loupe", `acct_1Ts8WKHRn8ZbPNdX`); build/test in a **sandbox** first, promote to live keys via Worker secrets.
...
- The subscriber pastes an opaque token (`wpck_live_…`) into the **same single-field UX** that already exists for API keys. `LLM::detect()` gains a prefix rule, `Settings` stores it under the existing `{provider}_api_key`-style slot, and `CHATADMIN_CLOUD_LICENSE` gives the constant-override path for free.
...
The proxy overrides `body.model → claude-haiku-4-5` past the cap and returns `X-ChatAdmin-Throttled: 1`; the plugin shows a gentle, dismissible notice. Revisit top-ups later if data shows demand.
...
4. Proxy injects `x-api-key: <master>` and forwards to `api.anthropic.com`, returning the response **verbatim** plus `X-ChatAdmin-Usage`, `X-ChatAdmin-Limit`, `X-ChatAdmin-Throttled`.
...
200 → native Anthropic messages response
      + X-ChatAdmin-Usage: 42  X-ChatAdmin-Limit: 150  X-ChatAdmin-Throttled: 0|1
402 chatadmin_subscription_inactive   # past_due beyond grace / canceled
403 chatadmin_site_mismatch | chatadmin_license_invalid
429 chatadmin_rate_limited            # upstream Anthropic backpressure
```
Errors are shaped so `AnthropicProvider::error_message()` still yields a readable string, with an extra `error.type=chatadmin_*` the plugin can special-case (e.g. show "Manage billing").
...
- **`class-cloud-provider.php` (new)** — `class CloudProvider extends AnthropicProvider`:
  - `id() = 'cloud'`, `label() = 'ChatAdmin Cloud'`, `default_model() = 'claude-sonnet-4-6'`, `models()` limited to Sonnet (+ Haiku as the throttle target, shown read-only).
  - override `endpoint()` → cloud base (const, filterable `chatadmin_cloud_base_url`, overridable via `CHATADMIN_CLOUD_BASE_URL` for staging).
  - override `headers($key)` → bearer license + site + request-id + `anthropic-version`.
  - `seam_filter() = 'chatadmin_cloud_http_response'`.
...
- **`class-settings.php`** — `get_api_key('cloud')` returns the license from `chatadmin_settings['cloud_api_key']` or the `CHATADMIN_CLOUD_API_KEY` constant (reuses the existing `CHATADMIN_{PROVIDER}_API_KEY` mechanism unchanged). Add a **Cloud** settings section: status line (Active / Past-due / Canceled), usage meter, "Manage billing" (Portal) + "Subscribe" (Checkout) buttons. Hide the model picker in Cloud mode (locked to Sonnet).
...
- **`class-rest.php`** — no change to `handle_chat` logic; it already calls `LLM::active()` and reads `Settings::get_api_key()`. Optionally surface `X-ChatAdmin-Throttled`/usage from the provider response into the chat payload so the UI can show the soft-cap notice.
...
**Phase 4 — Polish.** Usage meter UI, past-due/dunning UX (grace window before 402), license rotation/revocation, `X-ChatAdmin-Throttled` in-chat notice, optional trial. Update `README.md`/`readme.txt` roadmap + `PRIVACY.md`, retire `cloud-waitlist`.

**Test seams (all phases):** `CloudProvider` → `chatadmin_cloud_http_response` + `tests/MockCloud.php`; Worker → miniflare with mock Anthropic + mock Stripe. Neither CI nor local dev ever calls a paid API.
...
2. **License delivery on `checkout.session.completed`:** email it, show it on a hosted return page, or have the plugin auto-fetch it via the Checkout `session_id` on redirect back to `/chat-admin`? Affects the onboarding return flow.
